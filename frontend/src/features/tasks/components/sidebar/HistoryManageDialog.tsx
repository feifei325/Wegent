...
export default function HistoryManageDialog({ open, onOpenChange }: HistoryManageDialogProps) {
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    try {
      if (isSelectingAll) {
        await taskApis.deleteAllPersonalTasks()
        setIsSelectingAll(false)
        setSelectedTaskIds(new Set())
      } else {
        const ids = Array.from(selectedTaskIds)
        await taskApis.bulkDeleteTasks(ids)
        setSelectedTaskIds(new Set())
      }
      setAllTasks([])
      loadTasks(1, false)
  ...
  }, [selectedTaskIds, isSelectingAll, loadTasks, refreshPersonalTasks])
  ...
  }, [loadTasks, refreshPersonalTasks])
